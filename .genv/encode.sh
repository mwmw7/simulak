#!/usr/bin/env bash
# Download Seedance results + web-optimize into public/assets/video/
set -uo pipefail
cd /home/branden/simula_entertainment
mkdir -p public/assets/video assets-src/video

for name in actor-aria actor-kai actor-nova artist-echo artist-lumen; do
  f=".genv/$name.json"
  [ -f "$f" ] || { echo "✗ $name: no json"; continue; }
  url=$(jq -r '(if type=="array" then .[0] else . end) | (.url.result_url // .result_url // .url)' "$f")
  if [ -z "$url" ] || [ "$url" = "null" ]; then echo "✗ $name: no url"; continue; fi
